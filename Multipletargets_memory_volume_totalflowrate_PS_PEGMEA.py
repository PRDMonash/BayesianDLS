...
def generate_initial_data(n,target):
    ...
    print(f"your initial water flowrates is: {waterflowrate} and your your intital polymer flowrates: {polymerflowrate}")
...
#Initiliaze the loop with training data(flowrates of pumps and according sizes) and read out list
X_initial_tensor, y_initial_tensor, y_initial, best_observerd_value=generate_initial_data(n=4,target=target_sizelist[0])
print(f"the initial volumefractions and totalflowrates are: {X_initial_tensor} and given sizes of: {abs(y_initial)} while the best observed score to target size is {-best_observerd_value}")
